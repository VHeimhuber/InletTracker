...
InletTracker is a Google Earth Engine enabled open source python software package that first uses a novel least cost path finding approach to trace IOCE entrance channels along and across the berm, and then analyses the resulting transects to infer whether an entrance is open or closed. InletTracker is built on top of the imagery download and pre-processing functionality of the CoastSat toolbox [https://github.com/kvos/CoastSat].

![Alt text](readme_files/EntranceSat_method_illustration.gif)

The underlying approach of the InletTracker toolkit and it's performance is described in detail in the following publication, which is currently under review:

Valentin Heimhuber, Kilian Vos, Wanru Fu, William Glamore (submitted): InletTracker: A Google Earth Engine enabled open-source python tool for historic and near real-time monitoring of intermittent estuary entrances, 2021
...
You might have to hit 'enter' here at some point for the setup to go ahead (even though you might not be asked to do so by Anaconda) and this whole step can take upwards of 10 mins. All the required packages have now been installed in an environment called `inlettracker`. Now, activate the new environment:
...
![Alt text](https://github.com/VHeimhuber/InletTracker/blob/main/readme_files/EntranceSat%20site%20setup%20illustration.jpg)
...
- Set the InletTracker folder/directory as the working subdirectory
...
![Alt text](https://github.com/VHeimhuber/InletTracker/blob/main/readme_files/training_data_generator.png)
...
That's it - you have now successfully used the InletTracker toolbox to analyse the entrance state of an intermittent estuary or a similar landform.
